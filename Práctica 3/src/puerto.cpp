...
void llegada_barco()
{
  ...
  if (((tormenta == false) || (afectan_tormentas == false)) && (remolcador == LIBRE) && (atraques_libres > 0))
...
void fin_atraque()
{
  ...
  if ((tormenta == false) || (afectan_tormentas == false))
...
void fin_carga()
{
  ...
  {
    total_toneladas += 1000.0;
  }
  else if (nodo.reg_cola.tipo == 10)
  {
    total_toneladas += 2000.0;
  }
  else
  {
    total_toneladas += 3000.0;
  }
  if ((remolcador == LIBRE) && ((tormenta == false) || (afectan_tormentas == false))) //comienza a desatracar el barco
  {
    acum_rem_amarrado += reloj - tdus_rem;
    tdus_rem = reloj;
    remolcador = OCUPADO;
...
void fin_desatraque()
{
  ...
  if ((encola_lleg > 0) && ((tormenta == false) || (afectan_tormentas == true))) //comienza a atracar un barco
  {
    acum_lleg += (reloj - tdus_lleg) * encola_lleg;
    tdus_lleg = reloj;
    encola_lleg--;
...
void fin_simulacion()
{
  ...
  informe[cont_simu][11] = total_toneladas;
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 5)
  {
    printf("\n\nFormato Argumentos -> <numero_simulaciones> <num_atraques> <tormentas_afectan 1/0 1-afectan> <tviajevacio>\n\n");
    exit(1);
  }
  int tormentas = 0;
  sscanf(argv[1], "%d", &simulaciones);
  sscanf(argv[2], "%d", &num_atraques);
  sscanf(argv[3], "%d", &tormentas);
  sscanf(argv[4], "%f", &tviajevacio);

  if (tormentas == 1)
    afectan_tormentas = true;
  else
    afectan_tormentas = false;
