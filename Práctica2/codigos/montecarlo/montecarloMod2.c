...
int main(int argc, char *argv[])
{
    ...
    if (argc < 7)
    {
        ...
        printf("%s  <cantidad maxima pedida>  <dias a simular> <tipo de tabla [a,b,c]> <beneficio por unidad> <coste devolucion> <coste> \n", argv[0]);
        exit(-1);
    }
...
int mejor = -1;
    ...
    for (int s = 1; s <= cant_maxima; s++)
    {
        ...
        for (int h = 0; h < veces; h++)
        {
            ...
            if (s > demanda)
            {
                ganancia = demanda*beneficio-fmin(coste,(s-demanda)*y);
            }else
            {
        ...
        if(ganancia_esperada > mejorGanancia){
            mejorGanancia = ganancia_esperada;
            mejor = s;
        }
    ...
    printf("m:%d:%f",mejor,mejorGanancia);
    free(tablabdemanda);
};